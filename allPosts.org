...
are all fantatic, free tools.

*** My Prior Workflow

Originally, when ready to publish, I would save the ~org~ file[fn:write]--this would trigger
...
#+INCLUDE: ".git/hooks/post-commit" src init

#+NAME: gitPre
#+CAPTION: .git/hooks/pre-push
#+INCLUDE: ".git/hooks/pre-push" src init

Finally, having turned off ~org-hugo-auto-export-mode~ (see
[[https://ox-hugo.scripter.co/doc/auto-export-on-saving/][here]]), I'm now able
to make a commit--this will get pushed to my [[https://github.com/OolonColoophid][Github repository]], the site built,
an the site uploaded to [[ianhocking.com][ianhocking.com]].
...
[fn:ox-auto] This is set with the variable ~org-hugo-auto-export-mode~. I'd set
this in a dotfile in the blog directory.
