...
One of the trickiest concepts on my [[https://www.cs.kent.ac.uk/masters/courses/msc-compsci/][MSc in Computer Science]] is recursion, which
...
again, on the output of what it just did. Tricky to get your head around; even writing sentence that was difficult!
...
because there are too many elements in play. Whenever this has to me in the
past, the solution has been the same: find a better representation system. If
Roman numerals are making maths tricky, that's time to switch to Arabic.

There are helpful ways of thinking about recursion out there. Among many, I found [[https://medium.com/@daniel.oliver.king/getting-started-with-recursion-f89f57c5b60e][this article]], which
...
I've been trying to do, and thinking more about the algorithmic
perspective---what is this function actually trying to do? Well, it is trying to split
the problem into smaller pieces (recursion), and when the pieces can't be made
any smaller, the recursions reverse their sequence.

I wanted, however, to write a recursive program that tells me a bit about its
recursive journey, and for that I implemented ~merge sort~ algorithm[fn:vneuman]. This was a
...
The algorithm itself splits the list into sub lists until the lists are only of
one element each (the base condition). Then, it merges the sublists into sorted
