...
** TODO Fix .httaccess error again
:PROPERTIES:
...
** TODO Where Do People Go?
:PROPERTIES:
:EXPORT_FILE_NAME: where-do-people-go
:END:

Until recently, if a visitor went to ianhocking.com, they'd be redirected to my
WordPress blog of several years called This Writing Life. I recently wanted to
archive this, so that I could give prominence to my new programmer blog.
However, I could never quite get the two blogs separate. WordPress kept
intercepting 404 errors and taking people to my old blog, whereas I wanted my
new blog's 404 page to appear.

I had a frustrating time with .htaccess files---success appeared intermittently,
basically because I was having trouble fully understanding the rewrite rules
used by WordPress. In a nutshell, WordPress alters the .htaccess file for the
...
dealing with permalinks. However, it also meant that my 'clever' solution wasn't
going to work.

Here is the relevant portion of my .htaccess file, with
explanations of what each of these lines does over at
...
The new base URL for This Writing Life is [[https://ianhocking.com/twl]]
